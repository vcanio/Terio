...
import { useClinicalUserStore } from "@/features/users/store/useClinicalStore"; // Importar
import { Plus, Trash2, Calendar, MapPin, FileText, AlertCircle } from "lucide-react";
...
export const VQDashboard = () => {
  ...
  const { activeUserId, users } = useClinicalUserStore(); // Obtener usuario activo
  ...
  // === FILTRADO ===
  // Solo mostramos sesiones que coincidan con el usuario activo
  const userSessions = activeUserId 
    ? sessions.filter(session => session.userId === activeUserId)
    : [];
  ...
  return (
    <div className="p-6 md:p-10 max-w-4xl mx-auto">
      ...
      {/* Alerta si no hay usuario seleccionado */}
      {!activeUserId && (
        <div className="mb-6 bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-xl flex items-center gap-3">
          <AlertCircle size={24} />
          <div>
            <strong>Modo sin usuario</strong>
            <p className="text-sm">Por favor selecciona un usuario en el Inicio para guardar tus datos correctamente.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-slate-500 text-sm">
            {activeUser ? `Historial de: ${activeUser.name}` : 'Registro de observaciones'}
          </p>
        </div>
        <button 
          onClick={() => setIsModalOpen(true)}
          disabled={!activeUserId}
          className="bg-blue-600 text-white px-4 py-2 rounded-xl font-medium flex items-center gap-2 hover:bg-blue-700 transition-colors shadow-lg shadow-blue-900/20 disabled:opacity-50 disabled:cursor-not-allowed"
        >
